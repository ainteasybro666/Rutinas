...
class RoutineEditFragment : BaseFragment(),
    ...
    @Inject // <<-- NUEVO: Inyectar AlarmScheduler -->>
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setupObservers() // Ahora llamamos a setupObservers aquí
...
//        calendarTriggerDialog = CalendarTriggerDialog()
        ...
        vb.btnSaveRoutine.setOnClickListener { // **ADAPTADO**
            ...
            // Llamar a saveRoutine() que ahora incluye la lógica de permisos
            saveRoutine()
        }
    ...
    override fun onDestroyView() {
        ...
        showBottomNavMenu()
        showFloatingButton()
        Timber.d("RoutineEditFragment: onDestroyView called.")
    }
    ...
    private fun setupObservers() {
        ...
        lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                // Observe UI events from ViewModel (like showMessage)
                launch {
                    viewModel.uiEvent.collect { event ->
                        Timber.d("RoutineEditFragment: Received UI event: $event")
                        when (event) {
                            is RoutineEditViewModel.UiEvent.ShowMessage -> {
    ...
    private fun saveRoutine() {
        ...
        if (validateForm(name, viewModel.triggers.value, viewModel.actions.value)) {
            Timber.d("RoutineEditFragment: Valid form. Checking for alarm triggers before saving.")
            ...
            if (hasAlarmTriggers && !alarmScheduler.canScheduleExactAlarms()) {
                // Si tiene triggers que requieren alarmas exactas y no tenemos el permiso, solicitarlo.
                Timber.d("RoutineEditFragment: Routine has alarm triggers and permission is missing. Requesting permission...")
                // TO-DO: Mostrar un diálogo explicativo al usuario antes de redirigirlo a la configuración.
                // Esto mejoraría la experiencia de usuario.
                requireView().showErrorSnackbar("Se requiere permiso de alarmas exactas para programar esta rutina. Por favor, concédelo en la configuración.")
                alarmScheduler.requestExactAlarmPermission()
                // No guardamos la rutina inmediatamente aquí. La idea es que el usuario vuelva
                // de la configuración y, si concedió el permiso, pueda intentar guardar de nuevo.
                // Podrías necesitar un mecanismo para reintentar el guardado al regresar,
                // o simplemente confiar en que el usuario intentará guardar de nuevo.
                return // No guardamos la rutina si no hay permiso
            }

            // Si no hay triggers de alarma o ya tenemos el permiso, guardar la rutina.
            Timber.d("RoutineEditFragment: No alarm triggers requiring permission, or permission already granted. Proceeding to save routine.")
            ...
            Timber.w("RoutineEditFragment: Routine validation failed. Save aborted.")
        }
    }
    ...
    override fun onTriggerSelected(triggerType: TriggerTypeDialog.TriggerType) {
        ...
        when(triggerType) {
            TriggerTypeDialog.TriggerType.TIME -> { // Usa el enum directamente
                // Usa el ID de la rutina actual si está disponible en el ViewModel, de lo contrario 0L
                val routineId = viewModel.currentRoutine.value?.id ?: 0L
                val timeDialog = TimeTriggerConfigDialog.createInstance(routineId)
            ...
            TriggerTypeDialog.TriggerType.CALENDAR -> { // Usa el enum directamente
                // Usa el ID de la rutina actual si está disponible en el ViewModel, de lo contrario 0L
                val routineId = viewModel.currentRoutine.value?.id ?: 0L
                val calendarDialog = CalendarTriggerDialog.createInstance(routineId)
            ...
            TriggerTypeDialog.TriggerType.LOCATION -> { // Usa el enum directamente
                // Usa el ID de la rutina actual si está disponible en el ViewModel, de lo contrario 0L
                val routineId = viewModel.currentRoutine.value?.id ?: 0L
                val locationDialog = LocationTriggerDialog.createInstance(routineId)
    ...
    override fun onCalendarTriggerConfigured(trigger: Trigger) {
        Timber.d("RoutineEditFragment: onCalendarTriggerConfigured received trigger: ${trigger.uuid}")
        ...
        when (val result = viewModel.addTrigger(triggerWithRoutineId)) {
            is Resource.Success -> {
                ...
                Timber.d("RoutineEditFragment: Calendar trigger añadido correctamente en la UI.")
            ...
            is Resource.Error -> {
                ...
                Timber.w("RoutineEditFragment: Error al añadir trigger de calendario: ${result.message}")
            }
    ...
    override fun onTimeTriggerConfigured(trigger: Trigger) {
        ...
        when (val result = viewModel.addTrigger(triggerWithRoutineId)) {
            is Resource.Success -> {
                // Trigger añadido correctamente. Puedes actualizar la UI si es necesario.
                Timber.d("RoutineEditFragment: Trigger de tiempo añadido correctamente en la UI.")
            ...
            is Resource.Error -> {
                // Mostrar el mensaje de error al usuario
                Toast.makeText(requireContext(), result.message, Toast.LENGTH_SHORT).show()
                Timber.w("RoutineEditFragment: Error al añadir trigger de tiempo: ${result.message}")
            }
        ...
        // viewModel.saveTrigger(trigger) // Comentado: La adición a la lista del ViewModel ya debería manejar esto internamente.
